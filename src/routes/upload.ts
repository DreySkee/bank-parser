...
router.post("/upload", upload.single("file"), async (req, res) => {
    try {
        ...
        const extension =
            ...
            `.${mime.extension(req.file.mimetype) || "pdf"}`;
            ...
            input: [
                {
                    role: "system",
                    content:
                        "You are a financial data parser. Output ONLY valid JSON, no explanations. Return a JSON array with fields: date, description, amount, type.",
                },
                {
                    role: "user",
                    content: [
